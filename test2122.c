...
int argc; char**argv;
{
	...
	t->_val = 10;
	t->_left = (tree) malloc(sizeof (struct _tree));
	t->_left->_val = 5;
	t->_right = (tree) malloc(sizeof (struct _tree));
	t->_right->_val = 15;
	list l2 = caminho(t, 15); // There may be problems due to the way
				  // the tree is being created
	...
	// Free the tree and free the list
	exit(0);
}
